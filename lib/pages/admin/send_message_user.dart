...
class SendMessageUser extends StatefulWidget {
  const SendMessageUser({Key? key}) : super(key: key);
...
class _SendMessageState extends State<SendMessageUser> with SingleTickerProviderStateMixin {
  ...
  Future<void> getInfo() async {
    ...
    querySnapshot.docs.forEach((doc) {
      var mapName = doc.get("name") as String?;
      var mapSurname = doc.get("surname") as String?;
      if (mapName != null && mapSurname != null) {
        uniqueItems.add("$mapName $mapSurname");
      }
    });

    setState(() {
      dropdownItems = uniqueItems.toList();
    });
  }
                  ...
                  onPressed: () async {
                    ...
                    if (selectedValue != null && _label.text != "") {
                      ...
                      await adminsRef.doc(adminId).update({
                        "yoneticiGelen": FieldValue.arrayUnion([ekle])
                      });
                      _label.clear();
